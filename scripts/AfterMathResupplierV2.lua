...
local function writeToFile(filename, data)
  ...
  if file == nil then
    ...
    gui.show_error("Aftermath", "Failed to write to " .. filename)
    return false
  end
...
local function checkAndCreateConfig(default_config)
  local config = readFromFile("AftermathResupplier.json")
  if config == nil then
    log.warning("Config file not found, creating a default config")
    gui.show_warning("Aftermath", "Config file not found, creating a default config")
    if not writeToFile("AftermathResupplier.json", default_config) then
      return false
    end
  ...
  if not writeToFile("AftermathResupplier.json", config) then
    return false
  end
  return true
end
...
local function readAndDecodeConfig()
  while not checkAndCreateConfig(default_config) do
    ...
    log.debug("Waiting for AftermathResupplier.json to be created")
  end
  return readFromFile("AftermathResupplier.json")
end
...
local function saveToConfig(item_tag, value)
  ...
  if t then
    t[item_tag] = value
    if not writeToFile("AftermathResupplier.json", t) then
      log.debug("Failed to encode JSON to AftermathResupplier.json")
    end
  end
end
...
local function readFromConfig(item_tag)
  ...
  if t then
    ...
    log.debug("Failed to decode JSON from AftermathResupplier.json")
  end
end
-------------------------------------------------------------------------------
...
if tonumber(online_version:get_string()) == 3274 then 
  ...
  aftermath_resupplier:add_imgui(function()
    if NETWORK.NETWORK_IS_SESSION_STARTED() then
      local MPx = "MP"..stats.get_character_index()
      ImGui.BeginTabBar("AftermathResupplier", ImGuiTabBarFlags.None)
      ...
      if ImGui.BeginTabItem("Production Overview") then
        ...
        if ImGui.IsItemHovered() and ImGui.IsItemClicked(0) then
          script.run_in_fiber(function()
            ...
            gui.show_message("AftermathResupplier", "Nightclub popularity increased.")
          end)
        end
        ImGui.EndTabItem()
      end
    else
      ImGui.Text("AftermathResupplier doesn't work in Single Player.")
    end
  end)
elseif tonumber(online_version:get_string()) > 3258 then
  gui.show_message("AftermathResupplier", "AftermathResupplier is not up-to-date.\nPlease update the script!")
  aftermath_resupplier = gui.get_tab("AftermathResupplier")
  aftermath_resupplier:add_text("AftermathResupplier is not up-to-date.\n\nPlease update the script.")
else
  gui.show_message("AftermathResupplier", "Failed to load!")
end