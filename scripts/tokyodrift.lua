...
tokyodrift               = gui.get_tab("GUI_TAB_LUA_SCRIPTS") --:add_tab("Tokyo Drift")
local popsnd, sndRef
local flame_size
...
tokyodrift:add_imgui(function()
 manufacturer = VEHICLE.GET_MAKE_NAME_FROM_VEHICLE_MODEL(ENTITY.GET_ENTITY_MODEL(current_vehicle))
  ...
  if ImGui.BeginPopupModal("Credits", true, ImGuiWindowFlags.AlwaysAutoResize | ImGuiWindowFlags.NoMove) then
    ImGui.Text("¤ Converted for Aftermath by ToXic (sofakingwetodddid).\10¤ Written by SAMURAI (xesdoog).\10¤ Credits to Harmless05 for the drift options [Shift-Drift].")
